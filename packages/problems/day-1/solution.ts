...
async function readInput() {
    return `${await readFile(resolve(__dirname, process.argv[2] === 'test' ? 'test_input.txt' : 'input.txt'))}`
        .trim()
        .split(/\r\n?/);
}
...
function calcWindow(readings: number[], window: number, index: number) {
    return (
        readCache(readings, window, index) ||
        setCache(
            readings,
            window,
            index,
            Array.from(new Array(window).keys()).reduce((acc, idx) => acc + readings[index - idx], 0),
        )
    );
}
